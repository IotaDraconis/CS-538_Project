...
def add_vehicle(plexe, vid, position, lane, speed, vtype="vtypeauto", route="platoon_route"):
    if plexe.version[0] > 1:
        traci.vehicle.add(vid, route, departPos=str(position),
                          departSpeed=str(speed), departLane=str(lane),
                          typeID=vtype)
    else:
        traci.vehicle.add(vid, route, pos=position, speed=speed,
                          lane=lane, typeID=vtype)


def add_platooning_vehicle(plexe, vid, position, lane, speed, cacc_spacing,
                           real_engine=False, vtype="vtypeauto", route="platoon_route"):
    """
    ...
    model
    """
    add_vehicle(plexe, vid, position, lane, speed, vtype, route)
